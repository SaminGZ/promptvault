// PromptVault offline service worker — build 20260917161221
const CACHE = 'pv-20260917161221';
...
self.addEventListener('fetch', e => {
  ...
  if(e.request.mode === 'navigate' || url.pathname.endsWith('index.html') || url.pathname.endsWith('sw.js')
    ...
    e.respondWith(fetch(e.request, {cache:'no-store'}).then(r => { caches.open(CACHE).then(c => c.put(e.request, r.clone())); return r; })
